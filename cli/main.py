...
== 运维子命令 ==
  web     子进程启动 gui/backend/api.py 的 Web 服务（页面 + /api/* 接口，默认 8001，
          占用报错；--forever 崩溃自动重启）
  pm2     管理 order-engine / signal-api / csi-flow 的 PM2 进程（非 JSON）
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ppm2 = sub.add_parser(
        "pm2",
        help="管理 order-engine / signal-api / csi-flow 的 PM2 进程",
    )
    ppm2.add_argument("pm2_args", nargs=argparse.REMAINDER,
                      help="order-engine|signal-api|csi-flow|save 及其参数")
